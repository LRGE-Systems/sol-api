module Pdf::BuyApproval::Classification
  class Base
    ...
    def dictionary
      {
        ...
        '@@execution_period@@' => "#{date_full} - #{ I18n.l(contract.created_at+deadline(lot).days, format: :date_contract) }"
      }
    end
    ...
    def fill_table_row(lot_group_item_lot_proposal)
      "<tr>"\
        "<td>#{lot_group_item_lot_proposal.item.id}</td>"\
    ...
    def template_data
      @template_data ||=
        File.read(
          Rails.root.join('lib', 'pdf', 'buy_approval', 'classification', 'templates', template)
        )
    end
