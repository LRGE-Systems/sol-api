module Pdf::OrderService::Classification
  class Base
    ...
    def dictionary
      {
        ...
        '@@classification_name@@' => bidding.classification.name,
        '@@bidding_descrition@@' => bidding.description,
        '@@total_value@@' => formatted_total_value,
        '@@total_full_value@@' => formatted_total_value,
        '@@number_covenant@@' => bidding.covenant.number,
        '@@covenant_name@@' => bidding.covenant.name,
        ...
        '@@execution_period@@' => "#{date_full} - #{I18n.l(contract.created_at+deadline(lot).days)}"
      }
    end
    ...
    def fill_table
      rows_table.each_with_index.map do |rows, index|
        table_items = "#{annexs_html[index]}<table class='table' align='center' style='border:0;'>#{table}"
    ...
    def fill_table_header
      "<tr>"\
        ...
        "<th style='border: 1px solid black;'>#{I18n.t("document.pdf.contract.table.header.title")}</th>"\
        "<th style='border: 1px solid black;'>#{I18n.t("document.pdf.contract.table.header.description")}</th>"\
        "<th style='border: 1px solid black;'>#{I18n.t("document.pdf.contract.table.header.classification")}</th>"\
        "<th style='border: 1px solid black;'>#{I18n.t("document.pdf.contract.table.header.unit")}</th>"\
        "<th style='border: 1px solid black;'>#{I18n.t("document.pdf.contract.table.header.quantity")}</th>"\
        "<th style='border: 1px solid black;'>#{I18n.t("document.pdf.contract.table.header.unit_price")}</th>"\
        "<th style='border: 1px solid black;'>#{I18n.t("document.pdf.contract.table.header.price")}</th>"\
      "</tr>"
    end

    def fill_table_row(lot_group_item_lot_proposal)
      "<tr>"\
        "<td style='border: 1px solid black;'>#{lot_group_item_lot_proposal.item.id}</td>"\
        "<td style='border: 1px solid black;'>#{lot_group_item_lot_proposal.item.title}</td>"\
        "<td style='border: 1px solid black;'>#{lot_group_item_lot_proposal.item.description}</td>"\
        "<td style='border: 1px solid black;'>#{lot_group_item_lot_proposal.item.classification.name}</td>"\
        "<td style='border: 1px solid black;'>#{lot_group_item_lot_proposal.item.unit.name}</td>"\
        "<td style='border: 1px solid black;'>#{formatted_number(lot_group_item_lot_proposal.lot_group_item.quantity)}</td>"\
        "<td style='border: 1px solid black;'>#{formatted_currency(lot_group_item_lot_proposal.price)}</td>"\
        "<td style='border: 1px solid black;'>#{price_total_lot(lot_group_item_lot_proposal)}</td>"\
      "</tr>"
    end
    ...
    def template_data
      @template_data ||=
        File.read(Rails.root.join('lib', 'pdf', 'order_service', 'classification', 'templates', template))
    end
